...
const GameContext = createContext<TGameContextProps>({
  level: 0, setLevel: ()=>{},
  board: [], setBoard: ()=>{},
  isComplete: false, setIsComplete: ()=>{},
  solutionBoard: [], setSolutionBoard: ()=>{},
  stateBoard: [], setStateBoard: ()=>{},
  solved: [], setSolved: ()=>{},
  checkCompletion: ()=>{},
  startGame: ()=>{},
});
...
async function getBoards():Promise<Array<TBoards>> {
  const boards = await(await (await fetch('boards.json',{
      headers : { 
...
const GameProvider = ({ children }:TGameProviderProps) => {
  ...
  const [solved, setSolved] = useState<Array<boolean>[]>(Array(2).fill(Array(9).fill(false)))
  ...
  const checkCompletion = ():void => {
    ...
    for(let i = 0; i < 9; i++){
      ...
      const currentRow:(string|number)[] = Array.from(new Set(solutionBoard.slice(activeRow * 9, (activeRow + 1)*9 - 1)))
      ...
      newStatus[0][activeRow] = currentRow.filter(val=>val !== '').length === 9
      newStatus[1][activeCol] = currentCol.filter(val=>val !== '').length === 9
      setSolved([...newStatus])
    }
  ...
  const startGame = useCallback((doReset: boolean = false) => {
    getBoards()
    .then(res=>{
      const data:Array<Array<number|null>> = res.map(item => item.board)
      const currentBoard:Array<number|string> = data[level]?.map(item=>item||'')
  ...
  useEffect(() => {
    if(localStorage.getItem(`level-${level}-solution`) === null) { 
      ...
      setSolutionBoard(JSON.parse(localStorage.getItem(`level-${level}-solution`) ||'')) 
    }
  }, [board, level])
